"""
Brain Task 3 Launch File for TidyBot2.

Launches the FULL system for Task 3: Liquid Pouring.

1. Simulation layer (via sim.launch.py):
   - MuJoCo bridge, Robot state publisher, Arm controllers, RViz
   - Default scene: scene_task3.xml (bottle assembly)
...
2. Brain layer (this file):
   ...
   - vision_node       (YOLO + Gemini object detection for navigation)
   - segment_task3     (Segmenting bottle body and lid for arms)
   - brain_node_task3  (Task 3 state machine orchestrator)

Usage:
ros2 launch tidybot_bringup brain_task3.launch.py scene:=scene_task3.xml show_mujoco_viewer:=true use_rviz:=true

Without sim:
ros2 launch tidybot_bringup brain_task3.launch.py use_sim:=false
...
def generate_launch_description():
    ...
    declare_scene = DeclareLaunchArgument(
        'scene', default_value='scene_task3.xml',
        description='MuJoCo scene XML file to load (default: scene_task3.xml for bottle assembly)'
    )
